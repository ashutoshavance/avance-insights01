...
import { TeamMember, getStrapiMedia } from '@/lib/strapi'

const FALLBACK_TEAM: TeamMember[] = [
  { id: 1, documentId: 'f1', name: 'Manoj Kumar',  role: 'Founder & CEO',        bio: 'Over 15 years of experience in market research and business strategy.' },
  { id: 2, documentId: 'f2', name: 'Sanjeev Kumar', role: 'Research Director',     bio: 'Expert in quantitative research methodologies and data analytics.' },
  { id: 3, documentId: 'f3', name: 'Amit Verma',   role: 'Operations Head',       bio: 'Manages pan-India field operations and quality control.' },
  { id: 4, documentId: 'f4', name: 'Neha Gupta',   role: 'Client Services Lead',  bio: 'Ensures seamless client communication and project delivery.' },
]
...
const milestones = [
  { year: '2021', title: 'Founded',              description: 'Avance Insights established in New Delhi' },
  { year: '2021', title: 'MRSI Membership',      description: 'Became a member of Market Research Society of India' },
  { year: '2021', title: 'Pan-India Expansion',  description: 'Extended operations to 15+ states' },
  { year: '2023', title: '100+ Projects',        description: 'Completed milestone of 100+ successful projects' },
  { year: '2024', title: 'ISO Certification',    description: 'Achieved ISO 9001:2015 certification' },
]
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="flex flex-col items-center gap-3 animate-pulse">
          ...
          <div className="h-3 bg-slate-200 rounded-full w-36" />
        </div>
      ))}
    </div>
  )
...
export default function AboutPage() {
  ...
  useEffect(() => {
    apiFetchSafe<StrapiListResponse<TeamMember>>(
      '/team-members?populate=*&sort=createdAt:asc',
      { next: { revalidate: 3600 } } as RequestInit
    ).then((res) => {
      setTeam(res?.data?.length ? res.data : FALLBACK_TEAM)
          ...
          {teamLoading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {team.map((member, index) => {
                const photoUrl = member.photo?.url ? getStrapiMedia(member.photo.url) : ''
                return (
                  <motion.div
                  ...
                  >
                    {photoUrl ? (
                      <div className="w-32 h-32 mx-auto rounded-full overflow-hidden relative mb-4 ring-4 ring-primary-100">
                        <Image
                          src={photoUrl}
                          alt={member.name}
                          fill
        ...
        <div className="container-custom">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={value.title} className="p-6 bg-white rounded-xl shadow-sm border border-secondary-100 hover:shadow-md hover:border-primary-200 transition-all duration-200">
